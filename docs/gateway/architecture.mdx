...
1. LLM calls `exec` with args `["git", "set", "mykey", "myvalue"]`
2. Gateway receives the tool call, logs it as a core `exec` call
3. Gateway runs `docker exec <container> sh -c "git commit mykey myvalue"`
