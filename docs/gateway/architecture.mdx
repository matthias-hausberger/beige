...
```json5
{
  ...
  agents: {
    travel: {
      ...
      tools: ["kv"],
      // Per-agent tool config overrides — deep-merged with tools.kv.config
      // toolConfigs: {
      //   kv: { denyCommands: ["del"] },
      // },
...
2. Calls `createHandler(config)` for each tool to create base handlers
3. For agents with `toolConfigs`, deep-merges the agent override with the base `tools.<name>.config` and calls `createHandler(mergedConfig)` to create a dedicated handler for that agent
4. Generates a launcher script per tool per agent
5. Mounts launchers read-only into `/tools/bin/`
6. Mounts tool packages read-only into `/tools/packages/`

At runtime, `ToolRunner` looks up the agent-specific handler first (if one exists), falling back to the base handler. Tools are unaware of this — they just receive the final merged config in `createHandler()`.
