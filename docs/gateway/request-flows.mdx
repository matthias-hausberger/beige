...
**Example:** `exec kv set trip:paris "March 15"`
...
sequenceDiagram
    ...
    LLM->>CT: exec("kv set trip:paris March 15")
    CT->>AL: log(agent, "core_tool", "exec", [...], "allowed")
    CT->>SM: exec(agent, ["sh", "-c", "kv ..."])
    SM->>SB: docker exec

    Note over SB,KV: Phase 2: Launcher → gateway socket → tool execution

    SB->>SB: Shell finds kv on PATH → launcher script runs
...
sequenceDiagram
    ...
    LLM->>LLM: tool_call: exec kv list
