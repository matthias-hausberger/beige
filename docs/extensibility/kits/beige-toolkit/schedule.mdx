...
| `maxConsecutiveFailures` | No | `-1` | How many consecutive execution failures a schedule is allowed before it is automatically marked as `"failed"` and stopped. `-1` means infinite — the schedule will never auto-fail due to errors (default). `0` fails on the very first error (no retries). Set to e.g. `2` to allow up to 2 retries (3 total attempts) before the schedule is disabled. The counter resets to `0` on every successful run. |
